...
*   🌳 **Abyss Skill Web**: A massive radial passive tree with 1,000 nodes, 6 disciplines, unique Notable/Keystone conversion effects, Jewel Sockets, stun immunity, and full individual node refund support.
...
*   🎼 **The tracks** are your own: drop your audio files into `runtime/music/` — bind-mounted read-only at `/audio` for both the Idely and TS3AudioBot containers. Idely scans that folder each run (`.mp3`, `.ogg`, `.opus`, `.flac`, `.wav`, `.m4a`), so whatever you put there is picked up automatically — no playlist config needed. The folder is git-ignored, so your library is never committed.
*   🙈 **Excluded from detection (all automatic):** the Idely client (by its own UID), the audio bots (by name *and* by UID, learned live from each snapshot), and the main poke bot (which publishes its own UID to the shared database on connect, so Idely excludes it without you copying anything). `IDELY_EXCLUDE_UIDS` remains for any extra identities you want ignored.
...
1.  Set `ENABLE_IDELY=true` in `config.env`, drop your music into `runtime/music/` (bind-mounted at `/audio` for both `ts3-idely` and `ts3audiobot`), and run `docker compose up -d`. That's the whole basic setup: Idely uses its own persistent client profile with a distinct identity, the TS3AudioBot config is seeded automatically, and the poke bot and audio bots exclude themselves from idle detection. No identities, UIDs or API tokens to copy by hand.

**Optional / advanced**

*   **Identity (`IDELY_IDENTITY`):** for servers that enforce a minimum identity security level, set this to a dedicated identity blob (must differ from the poke bot's). Otherwise Idely keeps the profile's own identity, persisted in the `idely_profile` volume across restarts.
*   **TS3AudioBot config:** the one-shot `ts3audiobot-init` service seeds a working `ts3audiobot.toml` (WebAPI enabled + `/audio` as the media path) and a permissive `rights.toml` into the `ts3audiobot_data` volume on first run, and never overwrites them afterwards. Edit those files in the volume to customise — e.g. add `generate_status_avatar = false` and `set_status_description = false` under `[bot.connect]` so bots don't push album art to their own icon. The talk-sensor plugin is bind-mounted at `/app/data/plugins`; Idely loads it per bot (`IDELY_PLUGIN=0`, the plugin index).
*   **WebAPI auth:** the seeded `rights.toml` lets Idely drive the WebAPI anonymously over the internal Docker network (the `ts3audiobot` service publishes no ports), so no token is needed. To require one instead, PM the bot `!api token` and set `IDELY_API_USER` / `IDELY_API_TOKEN`.

All tuning (`IDELY_IDLE_MINUTES`, `IDELY_MAX_BOTS`, `IDELY_VOLUME`, `IDELY_TALK_STOP_MS`, `IDELY_TALK_COOLDOWN_SEC`, `IDELY_ONLY_CIDS`, …) is in `example.env`. You supply the music, so make sure you have the right to play whatever you drop in `/audio` on your server.
