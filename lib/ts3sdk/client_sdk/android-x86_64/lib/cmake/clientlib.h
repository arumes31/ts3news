...
struct ClientUIFunctions {
    ...
    * @brief JSON events
    *
    * @param msg A stringified JSON object.
    **/
    ...
    */
     ...
     *
     * Informs you about newly visible clients after subscribing to a channel.
    ...
    */
    ...
    /**
     * @brief invoked when some ui-relevant event happened in a screenshare session, with it's according payload in json
     *
     * @param id stringified uint64 id of the peer connection
     * @param payload stringified json payload
     */
    void (*onScreenshareSessionEvent)               (uint64 id, const char* event_name, const char* payload);

    /**
    * @brief invoked by an asynchronous request from ts3client_postMessage
    *
    * @param stringified json of the reply
    * @param return_code a c string identifying the action that caused this event. This is the same string given as return_code to function calls that request an action
    */
    void (*onJsonReply)                                 (const char* json, const char* return_code);

    /**
       * @brief Invoked when a JSON Command should be sent to the matrix module
       *
       * @param stringified json of the event
       * @param return_code a c string identifying the event. A return_code is waiting to be resolved by ts3client_onMatrixMessage
       */
    void (*onSendCallToMatrix)                           (unsigned int ts_chat_id, const char* json, const char* return_code);

    /**
     * @brief Called when a proto command response is ready.
     *
     * @param data Serialized ClientCommandResponse protobuf bytes
     * @param size Size in bytes
     * @param return_code The return_code passed to ts3client_postProtoCommand
     */
    void (*onProtoResponse)                              (const void* data, size_t size, const char* return_code);

    /**
     * @brief Called when a proto event occurs (server events filtered by ACCESS_SDK).
     *
     * @param data Serialized ServerEvent protobuf bytes
     * @param size Size in bytes
     */
...
/**
 * @brief set an explicit render device whose output should serve as the AEC reference signal
 *
 * By default the SDK pairs AEC with the same mode/device used for playback on the connection, which only
 * works when capture and playback share a backend mode. Use this function when playback is routed through
 * a custom device (e.g. a game engine's audio mixer) but echo cancellation should still operate against the
 * audio that is actually rendered to the user's speakers. The named device must be a native render endpoint
 * (no custom-mode devices); on platforms whose backend supports system-level loopback (Windows WASAPI, Linux
 * PulseAudio) this is the device whose loopback capture is fed to the echo canceller.
 *
 * Call after @ref ts3client_openCaptureDevice and @ref ts3client_openPlaybackDevice on the connection. The
 * reference association is reapplied automatically when AEC is toggled or the capture device is reactivated.
 * Pass empty strings for modeID and renderDeviceID to clear an explicit reference and fall back to the
 * default mode-match behaviour.
 *
 * @param serverConnectionHandlerID the connection handler whose AEC reference to set
 * @param modeID utf8 encoded c-string of the backend mode that owns the reference device (e.g. "windowsaudiosession", "pulseaudio")
 * @param renderDeviceID utf8 encoded c-string of the render device whose output should be used as AEC reference. Pass an empty string to use the connection's currently-open playback device.
 * @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason. Returns ERROR_not_implemented on platforms whose backend cannot loopback-capture an arbitrary render device (currently macOS).
*/
EXPORTDLL unsigned int ts3client_setAECReferenceDevice(uint64 serverConnectionHandlerID, const char* modeID, const char* renderDeviceID);
...
EXPORTDLL unsigned int ts3client_createAudioPlaybackHandle(uint64 scHandlerID, anyID* handle);
EXPORTDLL unsigned int ts3client_pauseAudioPlaybackHandle(uint64 scHandlerID, anyID handle, int pause);
EXPORTDLL unsigned int ts3client_closeAudioPlaybackHandle(uint64 scHandlerID, anyID handle);
EXPORTDLL unsigned int ts3client_enqueueAudioPlaybackHandle(uint64 scHandlerID, anyID handle, float* buffer, int buffer_size);
...
/**
 * @brief Request to delete one or more messages from an attached matrix room.
 *
 * @param serverConnectionHandlerID the connection handler on which to delete the message
 * @param roomAlias a utf8 encoded c string with the room alias of the channel to delete the message in
 * @param messageIds an array of utf8 encoded c strings containing the message ids to delete.
 * @param returnCode a c string to identify this request in callbacks. Pass an empty string if unused.
 * @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
 */
EXPORTDLL unsigned int ts3client_requestDeleteChannelTextMsg(uint64 serverConnectionHandlerID, const char* roomAlias,
                                                             const char** messageIds, size_t messageCount, const char* returnCode);
/**
 ...
 *
 * @param scHandlerID server connection handler ID
 * @param channelID channel ID
 * @param channelPW channel password (can be NULL)
 * @param objectKeys array of object keys to request URLs for
 * @param verbs array of HTTP verbs corresponding to each object key
 * @param contentLengths array of content lengths for upload operations
 * @param numObjects number of objects in the arrays
 * @param returnCode a c string to identify this request in callbacks. Pass an empty string if unused.
 * @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
 */
EXPORTDLL unsigned int ts3client_s3ft_getPresignedUrls(uint64 scHandlerID, uint64 channelID, const char* channelPW, const char** objectKeys, const char** verbs, const size_t* contentLengths, int numObjects, const char* returnCode);
...
* @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
*/
EXPORTDLL unsigned int ts3client_s3ft_deleteFile(uint64 scHandlerID, uint64 channelID, const char* channelPW, const char* objectKey, const char* returnCode);
...
* @param channelID channel ID
* @param channelPW channel password (can be NULL)
* @param oldObjectKey original object key
* @param newObjectKey new object key
* @param returnCode a c string to identify this request in callbacks. Pass an empty string if unused.
* @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
*/
EXPORTDLL unsigned int ts3client_s3ft_renameFile(uint64 scHandlerID, uint64 channelID, const char* channelPW, const char* oldObjectKey, const char* newObjectKey, const char* returnCode);
...
* @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
*/
EXPORTDLL unsigned int ts3client_s3ft_listFiles(uint64 scHandlerID, uint64 channelID, const char* channelPW, const char* returnCode);
...
* @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
*/
EXPORTDLL unsigned int ts3client_s3ft_uploadDoneNotification(uint64 scHandlerID, uint64 channelID, const char* returnCode);
...
* @param channelID channel ID
* @param channelPW channel password (can be NULL)
* @param objectKey object key to upload
* @param contentLength size of the file to upload
* @param returnCode a c string to identify this request in callbacks. Pass an empty string if unused.
* @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
*/
EXPORTDLL unsigned int ts3client_s3ft_getUploadUrl(uint64 scHandlerID, uint64 channelID, const char* channelPW, const char* objectKey, size_t contentLength, const char* returnCode);
...
* @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
*/
EXPORTDLL unsigned int ts3client_s3ft_getDownloadUrl(uint64 scHandlerID, uint64 channelID, const char* channelPW, const char* objectKey, const char* returnCode);
...
/**
 * @brief get the legacy UUID (SHA1) of the virtual server.
 *
 * @param serverConnectionHandlerID specifies the server on which to retrieve the property
 * @param result address of a variable to receive the legacy uuid value on success.
 * Memory is allocated by the client lib and must be freed by caller using @ref ts3client_freeMemory
 * @return An Error code from the @ref Ts3ErrorType enum indicating either success or the failure reason
*/
EXPORTDLL unsigned int ts3client_getServerLegacyUUID(uint64 serverConnectionHandlerID, char** result);
#ifdef __cplusplus
}
#endif
