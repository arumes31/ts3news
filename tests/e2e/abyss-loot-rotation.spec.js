...
test('weekly featured drops disclose exact weighting and UTC reset responsively', async ({ page }) => {
  ...
  const surface = page.locator('.abyss-loot-signals');
  const panel = surface.locator('#abyssFeaturedDrops');
  ...
  // Featured Drops shares the opaque surface with the pity and streak meters.
  await expect(surface).toHaveCSS('background-color', 'rgb(11, 16, 24)');
