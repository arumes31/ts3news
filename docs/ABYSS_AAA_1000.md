...
- [x] **AAA-0057 / 57** — every visible hostile health bar marks the strict 30% execute boundary and gains an explicit execute state below it; the combat engine emits a locale-independent, safely rendered transition line for direct, chain, cleave, pet, rescued-delver, parry, thorns, poison, hazard, storm, and overtime damage, while concealed-health encounters reveal neither state nor threshold (`xp_abyss_combat.go`, `xp.go`, `web_abyss_stage_effects.go`, `webassets/abyss_live.html`, `webassets/abyss_live.css`, `webassets/abyss.html`, `webassets/abyss_ui200.css`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0058 / 58** — a deterministic server-owned next-combat signal previews one enforced Summoner, Deathburst, Regenerator, or Berserker mechanic for the next depth; natural and Deep Drums boss floors advertise their existing interruptible summon ritual, Blind conceals all intel, and single/live/planned combat confirms the signal through the real spawned encounter and safe responsive UI (`abyss_enemy_forecast.go`, `abyss_enemy_forecast_test.go`, `web_abyss.go`, `web_abyss_enemies.go`, `webassets/abyss_enemy_forecast.html`, `webassets/abyss_enemy_forecast.css`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0059 / 59** — successful casts of three distinct regular skill IDs across the party unlock a server-authoritative 5% floor-XP bonus, rounded up to at least one XP; repeats and invalid casts cannot inflate it, automatic/live/planned fights share the same cast hook and settlement, and live combat exposes responsive 0/3–3/3 progress plus authoritative result receipts (`abyss_skill_variety.go`, `abyss_skill_variety_test.go`, `xp.go`, `web_abyss.go`, `web_abyss_live.go`, `webassets/abyss_skill_variety.html`, `webassets/abyss_skill_variety.css`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0060 / 60** — each Abyss combatant's opening hit receives a 1–20% bonus when the party wins the opening phase and that combatant's effective SPD exceeds the chosen target's frozen pre-action SPD; relative advantage scales at one-fifth strength, caps at double speed, and excludes enemy ambushes, ties, support openers, later waves, pets, chains, cleaves, and extra hits, while exact attacker/target SPD and bonus values receive a safe high-contrast callout in both combat logs (`abyss_first_strike.go`, `abyss_first_strike_test.go`, `xp.go`, `webassets/abyss_first_strike.css`, `webassets/abyss.html`, `webassets/abyss_live.html`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0061 / 61** — every identified equipped item exposes an accessible Armoury disclosure for its exact STR base-power and INT spell-scaling contribution, its rounded share of positive equipped gear stats, and the main-hand attack element with the engine's Physical fallback; penalties remain explicit, hidden items leak nothing, disclaimers separate gear contribution from final damage, and hover, click, Enter, Space, accordion, narrow-screen, and CSS delivery behavior are covered (`abyss_gear_damage.go`, `abyss_gear_damage_test.go`, `web_pages.go`, `web_abyss.go`, `webassets/abyss_gear_damage.html`, `webassets/abyss_gear_damage.css`, `webassets/abyss.html`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0062 / 62** — every Abyss combatant opens a fight with one non-regenerating Aegis equal to the lower of 10× DEF or 15% max HP; enemy attacks consume it before HP while hazards and other indirect damage bypass it, absorbed damage stays out of HP-damage totals without weakening thorns, and exact tactical/pixel bars, shield-only hit feedback, a break cue, reduced-motion behavior, combat logs, and summaries are covered (`abyss_shield.go`, `abyss_shield_test.go`, `xp.go`, `xp_abyss_combat.go`, `web_abyss_live.go`, `webassets/abyss_shield.html`, `webassets/abyss_shield.css`, `webassets/abyss_live.html`, `webassets/abyss_pixel.html`, `tests/e2e/abyss.spec.js`).
...
- [x] **AAA-0065 / 65** — a successful Abyss skill stun arms one non-stacking target-owned weakness window after the stunning action resolves; the next direct player hit against that enemy bypasses fumble and the normal roll for an overflow-safe 2× critical, consumes carried fumble recovery, and records the use, while pets, rescued allies, chains, cleaves, DoTs, hazards, and other indirect damage cannot spend it; exact logs, summaries, live effects, tactical/pixel markers, opening motion, reduced-motion and forced-color behavior, screen-reader wording, and JSON omission are covered (`abyss_weakness_window.go`, `abyss_weakness_window_test.go`, `xp.go`, `xp_abyss_combat.go`, `mobs.go`, `web_abyss_live.go`, `web_abyss_live_tactics.go`, `webassets/abyss_weakness_window.html`, `webassets/abyss_weakness_window.css`, `webassets/abyss_live.html`, `webassets/abyss_pixel.html`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0066 / 66** — persistent companion orders add Focus Target, Guard Me, and Free-for-All to the entry Build step and the live action bar: focus follows the owner's latest direct enemy target, guard intercepts 15% of direct-hit damage and retaliates against the latest attacker, free preserves independent targeting, and explicit live orders spend the player action while companions act immediately; settings save atomically, rolling clients retain their existing choice, dead companions reject orders, and combat logs, summaries, live effects, accessible help, responsive/forced-color styling, and backend/browser regressions are covered (`xp_abyss_pet_commands.go`, `xp_abyss_pet_commands_test.go`, `xp.go`, `xp_abyss_combat.go`, `web_abyss_combat_settings.go`, `web_abyss_live_actions.go`, `web_abyss_live_tactics.go`, `webassets/abyss.html`, `webassets/abyss_pet_commands.css`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0067 / 67** — the entry Build step exposes the engine's authoritative Frontline and Backline formation rules with their exact trade-offs; the normalized choice is fixed in run flags, restored by Same as last run, defaults safely to Frontline for legacy and malformed requests, and drives target priority, damage, mitigation, physical evasion, live position labels, and threat for every fight (`abyss_position.go`, `abyss_position_test.go`, `web_abyss.go`, `web_abyss_builds.go`, `web_abyss_setup_state.go`, `webassets/abyss.html`, `webassets/abyss_entry_planner.html`).
...
- [x] **AAA-0073 / 73** — planned 3–20-floor descent now accepts server-validated safeguards for strict low-HP, target-depth, and persisted Legendary+ escrow drops; checks stay inside the per-player lock, successful partial batches preserve ordered combat playback, duplicate Legendary conversions retain their rarity signal, and a compact responsive safety console reports the authoritative stop reason (`web_abyss_auto_descend.go`, `web_abyss_auto_descend_test.go`, `web_abyss.go`, `web_abyss_loot.go`, `xp.go`, `webassets/abyss.html`, `webassets/abyss_auto_descend.css`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0074 / 74** — every declared enemy combat effect now exposes stable server-owned icon, tone, and mechanic metadata in live snapshot schema v2; tactical cards show escaped named affix chips with native hover explanations, pixel combat mirrors them as distinct pips, the effective Vampiric Host modifier appears on every affected enemy without mutating combat state, and accessible target names, forced-color styling, mobile overflow, hostile markup, backend, and browser regressions are covered (`content/mob_effects.go`, `web_abyss_mob_affixes.go`, `web_abyss_live.go`, `web_abyss_live_tactics.go`, `webassets/abyss_live.html`, `webassets/abyss_pixel.html`, `webassets/abyss_mob_affixes.css`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0075 / 75** — direct Abyss player attacks now have an exact one-percent server-authored critical-fumble drama roll with four escaped recovery lines and explicitly no combat effect; the legacy three-percent half-damage and stored revenge-crit state are removed, cosmetic entropy is isolated from seeded combat randomness, and dedicated tactical/live log styling covers reduced motion, forced colors, hostile markup, and 360px overflow without expanding the combat CSS monolith (`abyss_critical_fumble.go`, `xp.go`, `abyss_weakness_window.go`, `webassets/abyss.html`, `webassets/abyss_live.html`, `webassets/abyss_critical_fumble.css`, `tests/e2e/abyss.spec.js`).
...
- [x] **AAA-0079 / 79** — eligible Abyss-pool gear rolls gain a 25% server-owned completion bias when exactly three distinct pieces of a four-plus-piece named set are owned across equipped, backpack, and active escrow gear; the replacement preserves rarity, respects category chase, stops after the fourth piece, cannot compete with smart loot or cross Starter/Standard/Insanity boundaries, and persists a generic unidentified-safe `SET PITY · 3→4` reason for initial and live-refresh manifests (`web_abyss_loot.go`, `web_abyss_inventory_ui.go`, `webassets/abyss.html`, `webassets/abyss_inventory_ui.html`, `webassets/abyss_loot_presentation.css`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0080 / 80** — identified catalog gear permanently unlocks an account-wide appearance through the existing cosmetic ledger; the Forge Wardrobe filters exact-slot skins, posts rarity-scaled gold prices, atomically applies or freely clears only `Gear.AppearanceID`, rejects live combat, locked-out ownership and incompatible slots, preserves every power field, exposes the cosmetic identity in Armoury and Inventory, and includes responsive, reduced-motion, forced-color, transaction, rollback, and browser coverage (`artifacts.go`, `web_abyss_transmog.go`, `web_pages.go`, `webassets/abyss_transmog.html`, `webassets/abyss_transmog.css`, `tests/e2e/abyss.spec.js`).
...
- [x] **AAA-0087 / 87** — three distinct Abyss-exclusive catalog items rotate deterministically each ISO week and receive exactly 2× candidate weight in eligible Abyss rolls after category and duplicate filtering; rarity, smart-loot, set-pity, Starter, Standard, and Insanity boundaries stay intact, while a responsive hero panel publishes names, slots, rarity, exact weighting, week, and UTC reset (`abyss_weekly_featured_drops.go`, `web_abyss_loot.go`, `webassets/abyss_featured_drops.html`, `webassets/abyss_featured_drops.css`, `tests/e2e/abyss-loot-rotation.spec.js`).
...
- [x] **AAA-0095 / 95** — the Shop tab exposes identified-only Predator and Warden collection meters toward the server's existing four-piece completion safeguard, states the exact 25% compatible-roll bias at 3/4, and explicitly withholds unidentified set identity while warning that hidden gear can still affect authoritative resolution (`web_abyss_set_pity.go`, `webassets/abyss_set_pity.html`, `webassets/abyss_set_pity.css`).
- [x] **AAA-0096 / 96** — the known Workshop recipe crafts passive Abyss Insurance Charms; one charge transactionally returns 50% of an otherwise uninsured cache on defeat without occupying a combat carry slot, while grace, paid cover, Anchor Rune, Hardcore, Uninsured, and Abstinence retain priority and never burn the charm (`web_abyss_insurance_charm.go`, `web_abyss_econ.go`, `webassets/abyss_core_risk.html`, `web_abyss_insurance_charm_test.go`).
- [x] **AAA-0097 / 97** — identified rings can transactionally reroll to one through three sockets for five Void Shards; the bounded result never drops below the fitted gem count, preserves every gem and tier while rerolling their order, supports forge undo, and is exposed only for Finger1/Finger2 through the signed quote path (`web_abyss_ring_sockets.go`, `web_abyss_ring_sockets_test.go`, `webassets/abyss_ring_sockets.html`, `tests/e2e/abyss-ring-sockets.spec.js`).
- [x] **AAA-0098 / 98** — the first successful item identification each UTC day is free; the server-owned claim commits atomically with the item mutation, stale concurrent quotes fail closed, invalid items do not consume it, and Identify All discounts exactly one item (`web_abyss_daily_identify.go`, `web_abyss_daily_identify_test.go`, `webassets/abyss_daily_identify.html`, `tests/e2e/abyss-daily-identify.spec.js`).
...
- [x] **AAA-0129 / 129** — opt-in Auto-insure atomically applies ordinary 25% cache coverage during compatible run entry, consumes a ready free-cover voucher before gold, rolls the entire entry back when its premium is unaffordable, and visibly skips Hardcore or Uninsured runs without disabling the preference (`web_abyss.go`, `web_abyss_risk_rewards.go`, `web_abyss_shop_economy.go`, `webassets/abyss.html`, `0094_abyss_auto_insure.up.sql`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0130 / 130** — the Auction House exposes material listings and escrow-backed material buy orders.
- [x] **AAA-0131 / 131** — the Forge repair control exposes the authoritative linear 200g-per-missing-durability curve and the current exact full-repair total before opening its transactional quote (`web_abyss.go`, `webassets/abyss.html`, `web_abyss_economy4_test.go`, `tests/e2e/abyss.spec.js`).
- [x] **AAA-0132 / 132** — the Hall of Delvers retires gold through four permanent, cosmetic-only monuments; each design is limited to one per player, purchased under the existing per-player lock and a row-locked transaction, then displayed on a dedicated public plaza using only escaped nicknames and acquisition dates (`web_abyss_plaza.go`, `webassets/abyss_plaza.html`, `0095_abyss_plaza.up.sql`, `web_abyss_plaza_test.go`, `tests/e2e/abyss-plaza.spec.js`).
