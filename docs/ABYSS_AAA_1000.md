...
- [x] **AAA-0097 / 97** — Armoury and Inventory expose identified gear provenance through a native keyboard-and-touch acquisition receipt instead of title-only metadata; the opaque disclosure wraps safely, supports forced colors, requires no JavaScript, and remains absent for unidentified gear (`webassets/gear_provenance.css`, `webassets/armory.html`, `webassets/inventory.html`, `tests/e2e/abyss-inventory-polish.spec.js`).
