...
- [x] **AAA-0089 / 89** — every successfully escrowed Mythic, Celestial, or Eternal gear drop schedules one asynchronous, BBCode-neutralized server-wide TeamSpeak fanfare through a four-worker overload bound; sub-Mythic and failed persistence paths stay silent, while forge-created Eternal items retain their post-commit announcement (`abyss_drop_announcement.go`, `web_abyss_loot.go`, `web_abyss_features.go`, `web_abyss_forge_crafting.go`).
