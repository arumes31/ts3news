(function(){
 ...
 function subclass(){return shownClass&&shownClass.subclasses.find(s=>s.id===$('abyssSubclass').value);}
 ...
 function chooseClass(id){shownClass=data.catalog.find(c=>c.id===id)||data.catalog[0];const index=data.catalog.indexOf(shownClass);$('abyssClassPortrait').style.backgroundPosition='0% '+(index*20)+'%';$('abyssClassName').textContent=shownClass.name;$('abyssClassDescription').textContent=shownClass.description;
  ...
  $('abyssSubclass').replaceChildren(...shownClass.subclasses.map(s=>{const option=node('option',s.name);option.value=s.id;return option;}));
 ...
 function renderSubclass(){const sub=subclass();if(!sub)return;
  ...
  $('abyssClassPortrait').style.backgroundImage='url("/static/abyss_subclasses_'+(subIndex<6?'martial':'mystic')+'_v1.png")';
  $('abyssClassPortrait').style.backgroundPosition='0% '+((subIndex%6)*20)+'%';
  $('abyssClassName').textContent=shownClass.name+' / '+sub.name;

  $('abyssClassSequence').textContent=sub.sequence;$('abyssClassStats').textContent=sub.stats.join(' → ')+' · '+sub.resource+' 0–3';
  const selected=sub.id===data.state.selected;$('abyssClassChoose').textContent=selected?'Active subclass':'Use '+sub.name;$('abyssClassChoose').disabled=busy||data.locked||selected;
  $('abyssClassBuffs').textContent=sub.buffs;$('abyssClassGearAdvice').textContent=sub.gear;
  $('abyssClassSignatures').replaceChildren(...data.signatures[sub.id].map(skill=>{const row=node('div',undefined,'ab-class-signature'),title=node('div');title.append(node('h4',skill.name),node('small',skill.role+' · '+skill.mana+' mana · '+skill.cooldown+' round cooldown'));
 ...
 function render(){const active=data.catalog.find(c=>c.subclasses.some(s=>s.id===data.state.selected));
  ...
  $('abyssClassLegacy').disabled=busy||data.locked||!data.state.selected;
  const activeSub=active&&active.subclasses.find(s=>s.id===data.state.selected);
  status(data.locked?'Build locked for this run. Bank or end the run to switch.':activeSub?active.name+' / '+activeSub.name+' active. Switching between runs is free.':'Your original build is active. Inspect a class to choose a new combat style.');
 }
 async function load(){if(busy)return;busy=true;status('Loading your build…');try{data=await request();render();}catch(err){status(err.message);}finally{busy=false;if(data){renderSubclass();$('abyssClassLegacy').disabled=data.locked||!data.state.selected;}}}
 async function save(profile){if(busy||!data)return;busy=true;root.setAttribute('aria-busy','true');$('abyssClassChoose').disabled=true;$('abyssClassSaveSkills').disabled=true;
  try{data=await request({selected:profile===null?'':subclass().id,revision:data.state.revision,...(profile?{profile}: {})});render();status('Build saved. '+(data.state.selected?'Your class graphic and skills are ready for the next run.':'Your original build is active.'));}
  catch(err){status(err.message);}finally{busy=false;root.removeAttribute('aria-busy');if(data){renderSubclass();$('abyssClassLegacy').disabled=data.locked||!data.state.selected;}}
 }
