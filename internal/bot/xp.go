...
func (b *Bot) rollLootForUser(uid string, mob content.Mob, zoneDifficulty float64, focus string) (string, string) {
	...
	for i := 0; i < count; i++ {
		...
		if r < effUltChance {
			...
			_, _ = b.DB.Exec("UPDATE users SET title=$2, title_mult=$3, title_expires=NOW() + INTERVAL '7 days' WHERE client_uid=$1 AND (title IS NULL OR title_expires < NOW())", uid, t.Name, t.XPMultiplier)
