...
func abyssForgeOperationGuides() []abyssForgeOperationGuide {
	guides := []abyssForgeOperationGuide{
		...
		{Operation: "reforge", Summary: "Shape stats with lock-cost scaling and accept/reject thresholds. CRT/DGE soft caps are half-chance ratings, not percentage limits: 50 CRT gives 25% crit; 25 DGE gives 12.5% dodge.", OutcomeFamilies: []string{"offense", "defense", "speed", "utility", "balanced"}, Safeguards: []string{"lock up to two stats", "minimum quality", "compare before accept", "one-step rollback", "recalibration cap"}, SoftCaps: map[string]float64{"CRT": 50, "DGE": 25, "SPD": 250}, SupportsUndo: true, SupportsPreset: true},
