...
func (s *WebServer) autoConcedeIfTimedOut(w http.ResponseWriter, uid string, run abyssRun) bool {
	...
	s.abyssOps.funnel.observeEnd(uid, "timeout")
	var gold int64
...
func (b *Bot) abyssRaffleSettle(uid string) int64 {
	...
	if _, err := tx.Exec("/* economy:bot.Bot.abyssRaffleSettle */ UPDATE users SET gold = LEAST(9223372036854775807::numeric, gold::numeric + $1)::bigint WHERE client_uid=$2", pot, winner); err != nil {
		return 0
	}
	if err := tx.Commit(); err != nil {
		return 0
