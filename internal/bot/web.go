...
func (s *WebServer) Start(ctx context.Context, addr string) error {
	...
	if s.bot.Cfg.EnableAbyss {
		...
		mux.HandleFunc("/api/abyss/batch_temper", s.authAPI(s.handleAbyssBatchTemper))
		mux.HandleFunc("/api/abyss/temper_guard", s.authAPI(s.handleAbyssTemperGuard))
		mux.HandleFunc("/api/abyss/forge_queue", s.authAPI(s.handleAbyssForgeQueue))
		mux.HandleFunc("/api/abyss/gem_upgrade_all", s.authAPI(s.handleAbyssGemUpgradeAll))
		mux.HandleFunc("/api/abyss/scrape_rune", s.authAPI(s.handleAbyssScrapeRune))
		mux.HandleFunc("/api/abyss/unattune", s.authAPI(s.handleAbyssUnattune))
		mux.HandleFunc("/api/abyss/masterwork_transfer", s.authAPI(s.handleAbyssMasterworkTransfer))
		mux.HandleFunc("/api/abyss/reforge_lock", s.authAPI(s.handleAbyssReforgeLock))
		mux.HandleFunc("/api/abyss/rebalance_all", s.authAPI(s.handleAbyssRebalanceAll))
		mux.HandleFunc("/api/abyss/unbrand", s.authAPI(s.handleAbyssUnbrand))
		mux.HandleFunc("/api/abyss/special_reroll", s.authAPI(s.handleAbyssSpecialReroll))
		mux.HandleFunc("/api/abyss/awaken_guided", s.authAPI(s.handleAbyssGuidedAwaken))
		mux.HandleFunc("/api/abyss/imbue_remove", s.authAPI(s.handleAbyssImbueRemove))
		mux.HandleFunc("/api/abyss/polish_all", s.authAPI(s.handleAbyssPolishAll))
		mux.HandleFunc("/api/abyss/craft_repair_kit2", s.authAPI(s.handleAbyssCraftRepairKit2))
		mux.HandleFunc("/api/abyss/socket_relocate", s.authAPI(s.handleAbyssSocketRelocate))
		mux.HandleFunc("/api/abyss/fuse_preview", s.authAPI(s.handleAbyssFusePreview))
		mux.HandleFunc("/api/abyss/celestial_fuse_boosted", s.authAPI(s.handleAbyssCelestialFuseBoosted))
		mux.HandleFunc("/api/abyss/recipe_fav", s.authAPI(s.handleAbyssRecipeFav))
		mux.HandleFunc("/api/abyss/convert_mats", s.authAPI(s.handleAbyssConvertMats))
		...
		mux.HandleFunc("/api/abyss/tree/prestige_memory", s.authAPI(s.handleAbyssTreePrestigeMemorySet))
		mux.HandleFunc("/api/abyss/tree/loadout_save", s.authAPI(s.handleAbyssTreeLoadoutSave))
		mux.HandleFunc("/api/abyss/tree/loadout_apply", s.authAPI(s.handleAbyssTreeLoadoutApply))
		mux.HandleFunc("/api/abyss/tree/build_import", s.authAPI(s.handleAbyssTreeBuildImport))
		mux.HandleFunc("/api/abyss/tree/swap_keystone", s.authAPI(s.handleAbyssTreeSwapKeystone))
		mux.HandleFunc("/api/abyss/tree/jewel_fuse", s.authAPI(s.handleAbyssTreeJewelFuse))
		mux.HandleFunc("/api/abyss/tree/branch_refund", s.authAPI(s.handleAbyssTreeBranchRefund))
		mux.HandleFunc("/api/abyss/tree/undo", s.authAPI(s.handleAbyssTreeUndo))
	}
